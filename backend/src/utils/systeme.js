...
export const addContactToCommunity = async (email, communityId) => {
  try {
    const searchResponse = await axios.get(
      `${SYSTEME_API}?email=${encodeURIComponent(email)}`,
      {
    ...
    await axios.post(
      `https://api.systeme.io/api/communities/${communityId}/members`,
    ...
    console.error("Error adding contact to community:", error.message);
  }
};
